...
Honest scope (reported transparently by run_vqe_local):
  * Weak coupling (U/t=1): this HVA reaches ~1-2% of the ED energy -- a clean
    demonstration that the variational circuit works.
  * Strong coupling (U/t=8): the Neel-referenced HVA plateaus well above ED
    (tens of %). This is a known, genuine limitation -- strong-coupling Hubbard
    ground states are best approached from the Heisenberg limit with a dedicated
    state-preparation circuit (exactly what arXiv:2511.02125 does), which is
    beyond a basic hackathon VQE. We do NOT paper over this.
...
def run_vqe_local(lat: HubbardLattice, t, U, layers=2, maxiter=1500, restarts=6,
                  seed=7, verbose=True):
    ...
    result = {
        ...
        "vqe_particles": N, "vqe_double_occ": D, "ed_double_occ": ed["avg_double_occupancy"],
...
if __name__ == "__main__":
    ...
    run_vqe_local(lat, 1.0, 1.0, layers=2, restarts=6)
    run_vqe_local(lat, 1.0, 1.0, layers=3, restarts=6)
    print("Strong coupling (documented HVA limitation):")
    run_vqe_local(lat, 1.0, 8.0, layers=3, restarts=6)