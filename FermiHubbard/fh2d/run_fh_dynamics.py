"""
run_fh_dynamics.py  --  quench time dynamics.

From the half-filling Neel product state, evolve under the Hubbard Hamiltonian
and track average double occupancy, staggered magnetization, and per-site
density. Compares three levels:
  ED (exact, expm_multiply)  vs  Trotter (statevector)  vs  emulator shots
...
Writes fig2_quench_dynamics.png, fig3_density_heatmaps.png, and an ED-vs-circuit
comparison bar chart. Also prints the max %-deviation vs ED (the <5% pass check).
"""
...
def run(save=True, plot=True, with_shots=True, shots=2000):
    ...
    if plot:
        ...
        p2 = plotting.plot_density_heatmaps(ed, Lx, Ly)
        print(f"  wrote {p1}\n  wrote {p2}")
        if with_shots:
            # ED-vs-circuit snapshot at the final time
            k = steps - 1
            p3 = plotting.plot_ed_vs_circuit(
                ["<D>", "m_stag"],
                [ed["avg_double_occupancy"][k], ed["staggered_magnetization"][k]],
                [shots_traj["avg_double_occupancy"][k], shots_traj["staggered_magnetization"][k]],
                [shot_err["avg_double_occupancy"][k], shot_err["staggered_magnetization"][k]],
                ylabel="value at final time", title=f"ED vs emulator shots (t={ed['times'][k]:.2f}/t)",
                fname="fig7_ed_vs_circuit.png")
            print(f"  wrote {p3}")
