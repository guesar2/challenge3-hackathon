...
def ed_ground_state(lat: HubbardLattice, t: float, U: float, verbose=True):
    ...
    Returns a dict with energy (of H_Hubbard, mu removed), energy_per_site,
    total_particles, avg_double_occupancy, staggered_magnetization, and the
    per-site density / double-occupancy / Sz maps.
    """
    mu = U / 2.0
    H = jw.build_hubbard_sparse(lat, t=t, U=U, mu=mu)
    ...
    # Reported Hubbard energy adds the mu*N term back (H_Hubbard = H + mu*N).
    e_grand = float(w[0])
    e_hubbard = e_grand + mu * N

    result = {
        "Lx": lat.Lx, "Ly": lat.Ly, "t": t, "U": U, "mu": mu,
        "energy": e_hubbard,
        "energy_per_site": e_hubbard / lat.n_sites,
        "total_particles": N,
        "avg_double_occupancy": obs.avg_double_occupancy(gs),
        "staggered_magnetization": obs.staggered_magnetization(gs),
        "density_per_site": obs.density_per_site(gs),
        "double_per_site": obs.double_occupancy_per_site(gs),
        "sz_per_site": obs.sz_per_site(gs),
    }
    if verbose:
        print(f"  {lat.Lx}x{lat.Ly}  U/t={U/t:>4.1f}: "
              f"E={e_hubbard:+.6f} (E/N={e_hubbard/lat.n_sites:+.6f}), "
              f"<N>={N:.3f}, <D>={result['avg_double_occupancy']:.4f}, "
              f"m_stag={result['staggered_magnetization']:+.4f}")
    return result
