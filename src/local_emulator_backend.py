...
def submit_quench_batch(N, h_field, J, dt, step_counts, n_shots, device_name="H2-1LE",
    ...
    shape ({step_count: result_dict}), minus the qnexus-only job/project
    tracing fields (there's no hosted job to trace back to).
    """
    ...
    out = {}
    for steps, bitstrings in zip(step_counts, bitstring_lists):
        out[steps] = {
...
def submit_adiabatic_batch(N, h_targets, J, ramp_steps_by_target, dt_by_target, n_shots, h_init,
    ...
    out = {}
    for h_target, ramp_steps, dt, bitstrings in zip(
        h_targets, ramp_steps_by_target, dt_by_target, bitstring_lists
    ):
        out[h_target] = {
            "bitstrings": bitstrings,
            "device_name": device_name,
