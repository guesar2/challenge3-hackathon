...
def submit_quench_batch(N, h_field, J, dt, step_counts, n_shots, device_name="H2-1LE",
    ...
    """Local equivalent of qnexus_backend.submit_quench_batch. Same return
    shape ({step_count: result_dict}, with 'bitstrings' + 'bitstrings_x'
    for the Z- and X-basis measurement of each step count), minus the
    qnexus-only job/project tracing fields (there's no hosted job to trace
    back to).
    """
...
def submit_adiabatic_batch(N, h_targets, J, ramp_steps_by_target, dt_by_target, n_shots, h_init,
    ...
    """Local equivalent of qnexus_backend.submit_adiabatic_batch -- Z- and
    X-basis circuits for every h_target, batched into one local round trip.
    """
    color_edges = build_chain_color_edges(N)
    bases = ("z", "x")
    jobs = [
