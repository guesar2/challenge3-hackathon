...
# ---- noisy device run + zero-noise extrapolation (the new fig2 curves) ----
...
NOISY_TIMEOUT = 1800.0      # seconds the client waits for the batch. A
                            # TimeoutError is CLIENT-side only: the job keeps
                            # running on Nexus and the quota is already spent,
                            # so fh_zne.resume_noisy_zne() picks the results up
                            # afterwards instead of resubmitting.
