...
def hva_generators(lat: HubbardLattice, t: float, U: float):
    ...
    variational angle so that particle number is conserved:
      ...
      - int_groups: one group per site, holding that site's (-Z_up, -Z_dn, +Z_up
        Z_dn)/4 terms, i.e. the bare n_up n_dn generator with NO factor of U (all
        diagonal, trivially number conserving). See the comment in the body for
        why the U must not be folded in.
    ...
    int_groups = []
    for site in lat.sites:
        # NOTE: the generator is the BARE number operator n_up n_dn, NOT
        # U * n_up n_dn. The variational angle already carries the magnitude, so
        # folding U into the generator is a pure reparametrisation theta -> U*theta
        # for U != 0 -- but at U = 0 it annihilates the whole interaction layer and
        # the ansatz collapses to a hopping-only circuit.
        #
        # That collapse is fatal, and silently so. Every hopping generator is
        # exp(-i theta h) with h real symmetric, so acting on a real product state
        # (the Neel reference) it can only ever produce one-body correlations
        # <c^dag_i c_j> that are purely IMAGINARY. The hopping energy is
        # -t * sum 2*Re<c^dag_i c_j>, so it is identically zero for every choice of
        # angles: at U=0 the optimiser returned E = 0 against an exact -4.000.
        # The diagonal interaction layer is what supplies the relative phases that
        # make Re<c^dag_i c_j> non-zero, so it must stay active at every U.
        up_q = lat.qubit(site, UP)
        dn_q = lat.qubit(site, DOWN)
        group = [(pauli, c) for pauli, c in interaction_pauli_terms(up_q, dn_q)]
